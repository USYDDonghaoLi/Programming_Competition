...
P = Prime(10 ** 7 + 10)
Ps = P.primes
d = {v: i + 1 for i, v in enumerate(Ps)}
# print('P', len(Ps))
...
def solve(testcase):
    ...
    cnt = [0 for _ in range(len(Ps) + 10)]
    for num in nums:
        for a, b in P.dissolve(num):
            cnt[d[a]] += 1
    ...
    for _ in range(q):
        ...
        right = bisect_right(Ps, r) - 1
        # print(Ps[left:right + 1])
        print(cnt[right + 1] - cnt[left])
