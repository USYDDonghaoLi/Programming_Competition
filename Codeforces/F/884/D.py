...
def solve(testcase):
    ...
    for a in P.get_divisors(n):
        ...
        while t < n - a:
            for j in range(t, t + a):
                adj[j].add(j + a)
                adj[j + a].add(j)
                # print(j, j + a, adj[j], adj[a + j], a in adj[j + a])
            t += a
    # print('adj', adj)
    
    res = [-1 for _ in range(n + 1)]
    idxs = [i for i in range(1, n + 1)]
    # idxs.sort(key = lambda x: len(adj[x]), reverse = True)

    for idx in idxs:
        S = set([i for i in range(26)])
        for o in adj[idx]:
            try:
                S.discard(res[o])
            except:
    ...
    string = [chr(ord('a') + r) for r in res[1:]]
    print(''.join(string))
