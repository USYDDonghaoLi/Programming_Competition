...
def solve(testcase):
    ...
    for i in range(n - 1, -1, -1):
        ...
        for v in range(x, n + 1, x):
            ft.update(ID[v], to_add[v])
            for u in range(v + v, n + 1, v):
                if ID[v] < ID[u]:
                    to_add[u] += to_add[v]
            to_add[v] = 0
        # print(i, *[ft.query_sum(i, i) for i in range(1, n + 1)])
        
        for r, idx in queries[i + 1]:
            # print(i, r, idx)
            res[idx] = ft.query_sum(i + 1, r)
