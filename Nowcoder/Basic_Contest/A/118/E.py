...
dp = [[inf for _ in range(100010)] for _ in range(18)]
dp[0][1] = 0

for j in range(1, 100010):
    dp[1][j] = j - 1

for i in range(2, 18):
    dp[i][1] = 0
    for j in range(2, 100010):
        for k in range(1, 100010):
            jk = j * k
            if jk >= 100010:
                break
            dp[i][jk] = fmin(dp[i][jk], dp[i - 1][j] + k - 1)

# @TIME
def solve(testcase):
    n, m = MI()
    print(dp[fmin(n, 17)][m])
