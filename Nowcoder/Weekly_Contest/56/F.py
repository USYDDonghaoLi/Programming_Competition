...
def solve(testcase):
    ...
    def check(mid, i):
        tval = Ht.GetHash(0, mid + 1)
        if mid <= i:
            '''
            from 0 to mid
            from i - mid to i
            from n - 1 - (i - mid) to n - 1 - i
            '''
            # print(n - 1 - i, n - 1 - i - mid + 1)
            sval = Hs2.GetHash(n - 1 - i, n - 1 - i + mid + 1)
        else:
            '''
            from 0 to i:
                from 0 to i
                from n - 1 - i to n - 1
            '''
            sval = Hs2.GetHash(n - i - 1, n)
            LEN = mid - (i + 1) + 1
            sval = (sval * Hs2.mulpw[LEN] + Hs1.GetHash(i + 1, mid + 1)) % Hs1.mod
            
        
        # print(mid, i, sval, tval, Hash.calc('aab'))
        return sval == tval
