...
class BitVector:
    ...
    __slots__ = "cnum", "bit", "chunk","built","blocks"
    def __init__(self, N):
        self.cnum = (N + 255) >> 8
...
class WaveletMatrix:
    ...
    def prev_value(self, l: int, r: int, upper: int):
        """返回[l,r) 范围内 小于upper的数中最大的数，没有返回None"""
    ...
    def next_value(self, l: int, r: int, lower: int):
        """返回[l,r) 范围内 大于等于lower的数中最小的数，没有返回None"""
...
def solve(testcase):
    ...
    for _ in range(m):
        ...
        LEN = r - l + 1
        if k == 0:
            right = fmin(right, WM.quantile(l, r + 1, 0) - 1)
        elif k == LEN:
            left = fmax(left, WM.quantile(l, r + 1, LEN - 1))
        else:
            left = fmax(left, WM.quantile(l, r + 1, k - 1))
            right = fmin(right, WM.quantile(l, r + 1, k) - 1)

    if left == right:
        print(1, left)
    else:
        print(pow(right - left + 1, mod - 2, mod))
