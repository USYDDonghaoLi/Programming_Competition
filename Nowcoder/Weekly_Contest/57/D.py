...
def solve(testcase):
    ...
    while i < len(UP) and j < len(DOWN):
        ...
        res.append((u + 1, d + 1, 'Y'))
        yes += 1
        i += 1
        j += 1
    
    k = 0
    while i < len(UP) and k < len(ON):
        u = UP[i]
        o = ON[k]

        res.append((u + 1, o + 1, 'Y'))
        yes += 1
        i += 1
        k += 1
    
    while j < len(DOWN) and k < len(ON):
        d = DOWN[j]
        o = ON[k]

        res.append((d + 1, o + 1, 'Y'))
        yes += 1
        j += 1
        k += 1
