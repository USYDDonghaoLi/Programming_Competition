...
F = Factorial(6010, mod)

Sterling = [[0 for _ in range(6010)] for _ in range(6010)]

Sterling[0][0] = 1

for i in range(1, 6010):
    for j in range(1, i + 1):
        Sterling[i][j] = (Sterling[i - 1][j] * j + Sterling[i - 1][j - 1]) % mod
...
def solve(testcase):
    ...
    for L in range(1, n + 1):
        ...
        for j in range(1, L + 1):
            ans += Sterling[L][j] * A[j] % mod * F.inv[j + 1] % mod
            ans %= mod
        res += ans * (n - L + 1) * pw[n - L] % mod
        res %= mod
