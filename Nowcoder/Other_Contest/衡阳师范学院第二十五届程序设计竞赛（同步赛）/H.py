...
def solve(testcase):
    ...
    mp = defaultdict(lambda: [0 for _ in range(61)])
    ...
    for i, c in enumerate(C, 1):
        ...
        if i == 2:
            res[i] = tmp * pow(G[i], mod - 2, mod) % mod
        else:
            res[i] = (res[i - 1] * tmp % mod * pow(G[i - 1], i - 2, mod)) * pow(pow(G[i], i - 1, mod), mod - 2, mod) % mod
