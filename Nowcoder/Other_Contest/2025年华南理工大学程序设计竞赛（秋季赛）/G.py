...
def solve(testcase):
    ...
    mp = [defaultdict(bool) for _ in range(n)]

    @bootstrap
    def f(idx, remain):

        flag = False
        for factor in P.GetAllFactors(remain):
            newval = remain - factor
            if newval:
                if not newval not in mp[idx]:
                    yield f(idx, newval)
                if not mp[idx][newval]:
                    flag = True
                    break
            else:
                if idx + 1 == n:
                    ...
                    if A[idx + 1] not in mp[idx + 1]:
                        yield f(idx + 1, A[idx + 1])
                    if not mp[idx + 1][A[idx + 1]]:
                        flag = True
    ...
    print("CandidateMaster" if mp[0][A[0]] else "LegendaryGrandmaster")
