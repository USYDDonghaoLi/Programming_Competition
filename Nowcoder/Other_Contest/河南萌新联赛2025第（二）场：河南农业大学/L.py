...
def solve(testcase):
    ...
    # B = [randint(1, 10 ** 9) for _ in range(n)]
    ...
    #     for j in range(i, n):
    #         for k in range(j, n):
    #             check += A[i] * pow(B[j], mod - 2, mod) * (n // (i + 1)) * (n // (j + 1)) * (n // (k + 1)) + C[k]
    #             # print('check i j k', i, j, k, A[i] * pow(B[j], mod - 2, mod) * (n // (i + 1)) * (n // (j + 1)) * (n // (k + 1)) + C[k])
    #             check %= mod
    ...
    cur_sum = 0
    for j in range(n):
        cur_sum += B[j] * pa[j + 1] % mod
        pc.append(cur_sum % mod)
        '''
        k ranges in (j, n)
        '''
    ...
    # res = 0
    for i in range(n):
        res += (n // (i + 1)) * pc[i + 1] % mod
        res %= mod
        # print('set k', i, (n // (i + 1)) * pc[i + 1] )
