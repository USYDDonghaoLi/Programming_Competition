...
def solve(testcase):
    ...
    A = Hash.calc('nico')
    B = Hash.calc('niconi')
    C = Hash.calc('niconiconi')

    res = [0 for _ in range(n + 1)]

    for i in range(n):
        res[i + 1] = fmax(res[i + 1], res[i])

        if i >= 3 and H.GetHash(i - 3, i + 1) == A:
            res[i + 1] = fmax(res[i + 1], res[i - 3] + a)
        
        if i >= 5 and H.GetHash(i - 5, i + 1) == B:
            res[i + 1] = fmax(res[i + 1], res[i - 5] + b)
        
        if i >= 9 and H.GetHash(i - 9, i + 1) == C:
            res[i + 1] = fmax(res[i + 1], res[i - 9] + c)
