...
class BitMartix:
    ...
    def __init__(self, n):
        self.n = n
        self.A = [Bitset(n) for _ in range(n)]
    ...
    def __mul__(self, b):
        ...
        for i in range(self.n):
            for j in range(self.n):
                res.A[i][j] = (self.A[i] & bt.A[j]).count()
                # res.A[i][j] = (self.A[i] & bt.A[j]).count() & 1
        
        return res

    def pow(self, k):
        res = self.identity(self.n)
        base = BitMartix(self.n)
        for i in range(self.n):
            for j in range(self.n):
