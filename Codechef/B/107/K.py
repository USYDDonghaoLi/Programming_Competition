...
def solve(testcase):
    ...
    def calc(l, r, curl, curm):
        if l > r:
            ...
            mid = l + r >> 1
            freq[(curl, curm, 1)] += 1
            yield calc(l, mid - 1, curl, curm + 1)
            yield calc(mid + 1, r, curl + 1, curm)
    ...
    for a in A:
        ...
        for (lo, hi, eq), ct in freq.items():
            ways = F.comb(a - 2, lo - 1) * F.comb(n - a, hi) % mod * F.f[lo] % mod * F.f[hi] % mod * F.f[n - lo - hi - eq] % mod * ct % mod
            res += (a - 1) * a // 2 % mod * ways % mod

            ways = F.comb(a - 1, lo) * F.comb(n - a - 1, hi - 1) % mod * F.f[lo] % mod * F.f[hi] % mod * F.f[n - lo - hi - eq] % mod * ct % mod
            res += (n * (n + 1) // 2 - (a + 1) * a // 2) % mod * ways % mod             
            
            if eq == 1:
                ways = F.comb(a - 1, lo) * F.comb(n - a, hi) % mod * F.f[lo] % mod * F.f[hi] % mod * F.f[n - lo - hi - eq] % mod * ct % mod
                res += a * ways % mod
