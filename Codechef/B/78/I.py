...
def solve(testcase):
    ...
    def MAPPING(x, y): return [y[0] + x * y[1], y[1]]
    def COMP(x, y): return x + y
    
    sg = LazySegmentTree([[0, 1] for _ in range(n + 1)], OP, [0, 1], MAPPING, COMP, 0)

    for l, a in enumerate(A):
        while r < n:
            idx = st.bisect_left(A[r])
            if idx and A[r] % st[idx - 1] != 0:
                break
            if idx < len(st) and st[idx] % A[r]:
                break
            st.add(A[r])
            r += 1
        RIGHT[l] = r 
    ...
    idxs = sorted(idx, key = lambda x : queries[x][0], reverse = True)
    i = 0

    for left in range(n - 1, -1, -1):
        right = RIGHT[left]
        sg.apply(left + 1, right + 1, 1)
        while i < q and queries[idxs[i]][0] == left + 1:
            l, r = queries[idxs[i]]
            res[idxs[i]] = sg.prod(l, r + 1)[0]
            i += 1
