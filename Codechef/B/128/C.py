...
def solve(testcase):
    n, m, k = MI()
    pw = [1 for _ in range(n + 1)]
    for i in range(1, n + 1):
        pw[i] = pw[i - 1] * (k - 1) % mod
    
    pw2 = [1 for _ in range(n + 1)]
    for i in range(1, n + 1):
        pw2[i] = pw2[i - 1] * m % mod
    
    pw3 = [1 for _ in range(n + 1)]
    for i in range(1, n + 1):
        pw3[i] = pw3[i - 1] * (m - k) % mod
    
    pw4 = [1 for _ in range(n + 1)]
    for i in range(1, n + 1):
        pw4[i] = pw4[i - 1] * (m - 1) % mod
    '''
    右边不能是k,
    '''
    res = 0
    for i in range(n):
        ...
        #tmp = pw2[n - i - 1]
        '''
        左边j(< k)个大于等于k, (i-j)个小于k
        '''
        tmp = 0
        for j in range(k):
            # print('add', i, j, pw3[j] * pw[i - j] * F.comb(i, j) * pw2[n - i - 1] % mod)
            tmp += pw3[j] * pw[i - j] * F.comb(i, j) % mod
            tmp %= mod
        
        # if k - 1 >= i:
        #     print(tmp == pw4[i])
        
        res += tmp * pw2[n - i - 1] % mod
        res %= mod
    print(res)
