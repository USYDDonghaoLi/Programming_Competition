...
#         M = 0
#         cnt = 0
#         for p in permutations([j for j in range(1, n + 1)]):
#             tmp = 0
...
def solve(testcase):
    ...
    for i in range(1, n + 1):
        ...
        if b:
            need = a + 1
            # print(n, i, a, b, min(b, i), max(0, b - i + 1))
            res.append(F.f[n - need] * need % mod * F.f[a] % mod * (min(b, i) - max(0, b - i + 1) + 1) % mod)
        else:
            res.append(F.f[a] * F.f[n - a] % mod)
