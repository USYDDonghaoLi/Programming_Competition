...
mp = [[[0 for _ in range(7)] for _ in range(7)] for _ in range(7)]
def calc(k):
    for points in combinations([i for i in range(6 * 6)], k):
        uf = UnionFind(6 * 6)
        grid = [[0 for _ in range(6)] for _ in range(6)]
        s = points[0]
        rs = [0 for _ in range(6)]
        cs = [0 for _ in range(6)]
        ...
        if uf.size[uf.Find(s)] == k and rs[0] and cs[0]:
            r, c = 0, 0
            while r < 6:
                if rs[r]:
                    r += 1
...
def solve(testcase):
    ...
    for i in range(1, 7):
        ...
        for j in range(1, 7):
            if j > m:
                break
            res += mp[k][i][j] * (n - i + 1) % mod * (m - j + 1) % mod
            res %= mod
