...
def calc(i):
    if i == 1:
        ...
        S = sorted(list(calc(i - 1)))
        FIND = -1
        for num in S:
            # print('check', num, PRIME.divisors(num))
            for div in PRIME.divisors(num):
                if div == 1:
                    continue
                if num // div * (div + 1) not in S and num * (div + 1) not in S and num // div * (div + 1) <= 10 ** 9:
                    FIND = num
                    break
            if FIND != -1:
                break
