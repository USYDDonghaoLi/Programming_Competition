...
F = Factorial(3010, mod)

inv2 = [1 for _ in range(3010)]
t = pow(2, mod - 2, mod)
...
def calc(m):
    ...
    res = 0
    for i in range(1, m):
        res += inv2[m] * F.comb(m - 1, i - 1) % mod * calc(i) % mod
        res %= mod
    return res * pow(1 - inv2[m], mod - 2, mod) % mod
...
def solve(testcase):
    ...
    for i in range(n):
        for j in range(i + 1):
            res[i] += inv2[i] * F.comb(i, j) % mod * calc(n - j) % mod
            res[i] %= mod
