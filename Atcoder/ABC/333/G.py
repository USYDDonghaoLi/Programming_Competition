...
def compare(r, x, y):
    ...
    reminder = [(p * b * d - q * a * d) % mod1, (p * b * d - q * a * d) % mod2]
    modulo = [mod1, mod2]

    result = C.crt(modulo, reminder, mod1 * mod2)
    return result * 2 <= q

def find_fractions(N, frac):
    p, q = frac
    n1, n2 = 1, 0
    d1, d2 = 0, 1
    ...
    while q:
        quo = p // q
        max_q = (N - d2) // d1 if d1 else (N - n2) // n1
