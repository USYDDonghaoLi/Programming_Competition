...
def solve(testcase):
    ...
    factors = P.GetAllFactors(n, True)
    ...
    def check(num):
        ...
        for f in factors:
            sf = str(f)
            if f == 1 or f > num or '0' in sf:
                continue
            else:
                r = int(sf[::-1])
                if num % (r * f) == 0:
                    state, arr = check(num // (f * r))
                    if state:
                        return True, [sf] + arr + [sf[::-1]]
        return False, []
