...
def solve(testcase):
    ...
    invw = pow(sum(W) % mod, mod - 2, mod)
    probs = [w * invw % mod for w in W]
    ...
    for x in range(n):
        for y in range(m + 1):
            for z in range(k + 1):
                for c in range(k - z + 1):
                    dp[x + 1][y + (c != 0)][z + c] += dp[x][y][z] * fact.g[c] % mod * pow(probs[x], c, mod) % mod
                    dp[x + 1][y + (c != 0)][z + c] %= mod
    
    print(dp[n][m][k] * fact.f[k] % mod)
